...
KAFKA_CONSUMUER_GOURP_ID = "test_0"
...
class MyflatmapFunction(FlatMapFunction):
    ...
    def stat_failure_bag(self,json_str,tag:str):
        ...
        list_get=list(statis_one.get_statistics(name=name,period=period,stat_date=stat_date))
        if len(list_get)==0:
            info={}
            info[json_mysql['label']]=json_mysql['count_failure']
            statis_one.add_statistics(name=name,period=period,stat_date=stat_date,info=info)
        else:
            info=list_get[0].info
            info[json_mysql['label']]=json_mysql['count_failure']
            statis_one.update_statistics(name=name,period=period,stat_date=stat_date,info=info)
    ...
    def minute_to_day(self,minute_json:str):
        ...
        for daytime_int_one in self.day_temp_timer.keys():
            ...
            self.stat_success_bag(json_str=self.day_count_timer.get(daytime_int_one),tag=self.tag)
            self.stat_failure_bag(json_str=self.day_count_timer.get(daytime_int_one),tag=self.tag)
