...
class ConsumingFlatMap(FlatMapFunction):
    ...
    def fill_state(self, value, start_dt: datetime, end_dt: datetime, state: ValueState) -> None:
        ...
        for date in pd.date_range(start_dt,end_dt)[:-1]:
            try:
                ...
                if value['device'] in res and value['category'] in res[value['device']]:
                    val = merge_dicts(res[value['device']][value['category']],val)
            except IndexError:
                pass
            except Exception:
    ...
    def write_sql(self, device, category) -> None:
        def helper(period, dt, data):
            ...
            if res:
                ...
                self.statistics_action.add_statistics(name=self.tag,period=period,stat_date=dt,info={device:{category: data}})
    ...
    def update_state(self, value, state: ValueState):
        ...
        try:
            state.update(json.dumps(merge_dicts(bag_profile_summary, state_dict)))
        except:
...
class HandleCountFlatMap(FlatMapFunction):
    ...
    def fill_state(self, value, start_dt: datetime, end_dt: datetime, state: MapState) -> None:
        ...
        for date in pd.date_range(start_dt,end_dt)[:-1]:
            for status in val.keys():
                ...
                try:
                    res = self.statistics_action.get_statistics(name=name,period='daily',stat_date=date)[0].info
    ...
    def check_expiration(self, daydt: datetime, weekdt: datetime, monthdt: datetime) -> None:
        ...
        if not self.today_data.is_empty():
            if daydt>str_to_datetime(self.dt_today.value()):
                for status in ['success','failure']:
                    self.yesterday_data.put(status, self.today_data.get(status))
                    self.today_data.put(status, 0)
                self.dt_yesterday.update(self.dt_today.value())
                self.dt_today.update(datetime_to_str(daydt))
            elif str_to_datetime(self.dt_yesterday.value())<daydt<str_to_datetime(self.dt_today.value()):   
        ...
        if not self.this_week_data.is_empty():
            if weekdt>str_to_datetime(self.dt_thisweek.value()):
                for status in ['success','failure']:
                    self.last_week_data.put(status, self.this_week_data.get(status))
        ...
        if not self.this_month_data.is_empty():
            if monthdt>str_to_datetime(self.dt_thismonth.value()):
                for status in ['success','failure']:
                    self.last_month_data.put(status, self.this_month_data.get(status))
    ...
    def write_sql(self, label) -> None:
        def helper(period, dt, data: MapState):
            for status in ['success','failure']:
                name = self.tag.replace('status', status)
                res = list(self.statistics_action.get_statistics(name=name,period=period,stat_date=dt))
                if res:
                    info = res[0].info
                    info[label] = data.get(status)
                    self.statistics_action.update_statistics(name=name,period=period,stat_date=dt,info=info)
                else:
                    self.statistics_action.add_statistics(name=name,period=period,stat_date=dt,info={label:data.get(status)})
    ...
    def flat_map(self, value):
        ...
        for status in ['success','failure']:
            key = f"count_{status}"
            if value['daydt'] == str_to_datetime(self.dt_yesterday.value()): # data from yesterday
                self.yesterday_data.put(status, self.yesterday_data.get(status) + value[key])
            elif value['daydt'] == str_to_datetime(self.dt_today.value()):   # data from today
                self.today_data.put(status, self.today_data.get(status) + value[key])
            if value['weekdt'] == str_to_datetime(self.dt_lastweek.value()):   # data from last week
                self.last_week_data.put(status, self.last_week_data.get(status) + value[key])
            elif value['weekdt'] == str_to_datetime(self.dt_thisweek.value()):   # data from this week
                self.this_week_data.put(status, self.this_week_data.get(status) + value[key])
            if value['monthdt'] == str_to_datetime(self.dt_lastmonth.value()):  # data from last month
                self.last_month_data.put(status, self.last_month_data.get(status) + value[key])
            elif value['monthdt'] == str_to_datetime(self.dt_thismonth.value()):    # data from this month
                self.this_month_data.put(status, self.this_month_data.get(status) + value[key])
