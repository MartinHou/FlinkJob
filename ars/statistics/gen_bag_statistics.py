...
class HandleErrorFlatMap(FlatMapFunction):
    ...
    def stat_pod(self, json_str):
        ...
        if len(list_get) == 0:
            ...
            info[json_mysql['group']][json_mysql['error_stage']][json_mysql['error_type']] = json_mysql['count']
            statis_one.add_statistics(
                name=self.tag, period=period, stat_date=stat_date, info=defaultdict2dict(info))
        else:
        ...
        if day_json == None:
            self.day_data.put(
                ...
                json.dumps({
                    'daytime_int': daytime_int,
                    'daytime': new_dict['daytime'],
                    'group':new_dict['group'],
                    'error_stage': new_dict['error_stage'],
                    'error_type': new_dict['error_type'],
                    'count': new_dict['count'],
                }))
        else:
            day_json = json.loads(day_json)
            day_json['count'] += new_dict['count']
            self.day_data.put(daytime_int, json.dumps(day_json))
...
def analyse(env):
    ...
    stat_replay_error_bag_count_group_by_category = stream.filter(lambda x:x.type == 'replay' and x.error_type!='')\
        ...
        .flat_map(HandleErrorFlatMap(tag='REALTIME_stat_replay_error_bag_count_group_by_category'))
