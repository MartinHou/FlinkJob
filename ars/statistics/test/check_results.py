...
def get_cnt(engine):
    with engine.connect() as connection:
        prod_doc = pd.read_sql(sql, con=connection).astype({"name":str,"info": str})
...
res_local,res_prod = pd.DataFrame(get_cnt(local_engine)),pd.DataFrame(get_cnt(prod_engine))
# print(res_local,res_prod)
cmp = pd.merge(res_local,res_prod,on=['Name', 'Key'],how='outer',suffixes=('_local','_prod'))
